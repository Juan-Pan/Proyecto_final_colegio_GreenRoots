...
<html lang="es">
  ...
  <body>
	...
	$SQL = "SELECT documento, nombre, id_perfil, id_grupo, telefono,
			grupo.grupo, perfil.perfil
			FROM arbol
			INNER JOIN tronco
			INNER JOIN hoja
			WHERE 
			arbol.id_tronco LIKE tronco.id AND arbol.id_hoja LIKE hoja.id
			AND (ubicacion LIKE '%$patron%' OR nombre LIKE '%$patron%'
			OR fruto LIKE '%$patron%' OR tronco.tipo_tronco LIKE '%$patron%' OR hoja.tipo_hoja LIKE '%$patron%') 
				...
				<table class="table table-hover table-danger">
					<caption>Resultados encontrados: <?php echo $registros?></caption>
						<thead class="thead-dark">							
							<tr>							
                            <th scope="col">No.</th>	
							<th scope="col">Id hoja</th>	
							<th scope="col">id tronco</th>							
						...
						<tbody>																					
							<?php																	
                                ...
                                $sitio = $fila[7];
									...
									echo"<td><small>$fecha_creacion</small></td>";
